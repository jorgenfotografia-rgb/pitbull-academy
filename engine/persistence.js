...
Academy.createPersistence=function({state,storage,notify=()=>{}}){
...
function keepOriginal(raw,key){
  if(raw!==null&&storage.getItem(key)===null)storage.setItem(key,raw);
}
function load(){
  let raw=null;
  try{raw=storage.getItem(KEY);persistence.lastStored=raw}catch(e){persistence.issue='read';return fresh()}
...
function save(){
  ...
  try{
    if(storage.getItem(KEY)!==persistence.lastStored){persistence.issue='conflict';notify();return false}
    ...
    try{const serialized=JSON.stringify(state.current);storage.setItem(KEY,serialized);persistence.lastStored=serialized}
