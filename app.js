...
const state=Academy.createState({MODULES,SCENARIOS,PRODUCTS});
const {KEY,SCHEMA_VERSION,BACKUP_KEY,RECOVERY_KEY,DEFAULT_MODULE_ID,blankModuleProgress,fresh,record,uniqueResults,normalizeProgress,migrate}=state;
const persistenceAdapter=Academy.createPersistence({state,storage:localStorage,notify:()=>persistenceStatus()});
const persistence=persistenceAdapter.status;
const {keepOriginal,load}=persistenceAdapter;
let S=load();
let actionEpoch=0;
...
function save(){state.current=S;return persistenceAdapter.save()}
